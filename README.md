...
9. **Sales > Invoices** ekranında faturaları arama, durum ve partner filtresiyle listeleyebilir; admin kullanıcılar yalnızca `DRAFT` durumundakileri oluşturup düzenleyebilir ve durumu `ISSUED`, `PAID` veya `CANCELLED` olarak değiştirebilir. `ISSUED` durumunda cari hesap kaydı oluşur, `PAID` için kalan bakiyenin sıfır olması gerekir.
