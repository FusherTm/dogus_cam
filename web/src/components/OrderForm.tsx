...
const OrderForm: React.FC<Props> = ({ mode, initial, onSubmit, onCancel, error }) => {
  ...
  const calcLine = (item: OrderItemIn) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const schema = mode === 'edit' ? orderUpdateSchema : orderCreateSchema
    const payload = {
      ...form,
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      <label>
        ...
        />
        {errors.discount_rate && <span style={{ color: 'red' }}>{errors.discount_rate}</span>}
      </label>
